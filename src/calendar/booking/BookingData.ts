...
async function getMyCalendar(coords: Coords, radiusKm: Radius) {
    ...
    const availabilityDatesAndLocations = await Promise.all(
        bmvLocations.map(async (location) => {
            ...
            const slots = filterSlots(
                locationsData?.availabilityDates[todayDateStr]
            );
    ...
    for (let i = 0; i < 60; i++) {
        ...
        for (let j = 0; j < availabilityDatesAndLocations.length; j++) {
            const availabilityDatesAndLocation =
                availabilityDatesAndLocations[j];
            const { location, availabilityDates } =
                availabilityDatesAndLocation;
...
export const useBookingData = (
    ...
    if (loading) {
        ...
        return {
            ok: new Map(
                Array.from(byMonth).sort((a, b) =>
                    String(b[0]).localeCompare(a[0])
                )
            ),
        };
    }
